class Juego {
  ...
  method descuento(_descuento) { descuento = _descuento }
  method precio() = precio

  method precioFinal() = descuento.descuento(precio) 

  method precioAptoMenoresMondedaLocal(pais) = if(pais.esAptoMenores(self)) self.precio() * pais.monedaLocal()
}

class DescuentoDirecto {
  var porcentaje

  method descuento(precio) = precio - (precio * porcentaje)
}

class DescuentoFijo {
  var monto

  method descuento(precio) = (precio - monto).max(precio/2)
}

class Gratis {
  method descuento(precio) = 0 
}

class SinDescuento {
  method descuento(precio) = precio
}
